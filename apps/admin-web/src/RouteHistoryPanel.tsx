...
export default function RouteHistoryPanel({ client, buses, selectedBusId, setSelectedBusId }: { client: Client; buses: Bus[]; selectedBusId?: string; setSelectedBusId: (value: string) => void }) {
  ...
  const search = async () => {
    ...
    setLoading(true)
    try { setHistory(await client.route(selectedBusId, date.startOf('day').toISOString(), date.endOf('day').toISOString())) }
  ...
  return <Card>
    <Space wrap className="toolbar">
      ...
      <Tag color="blue">Daily mileage: {history?.dailyMileageKm.toFixed(2) ?? '0.00'} km</Tag>
    </Space>
